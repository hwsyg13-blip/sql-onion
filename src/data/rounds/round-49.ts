...
export const ROUND_49: QuizQuestion[] = [
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 19,
    "title": "아래 두 테이블 T1, T2 (T2.B 일부 NULL 포함) 에 대한 복수컬럼 IN 비교 결과에 대한 설명으로 옳은 것은?",
    "options": [
      "T1 의 모든 행이 그대로 반환된다.",
      "T2 에서 B 가 NULL 인 행은 IN 비교에서 매칭되지 않아 결과에서 제외된다.",
      "복수컬럼 IN 비교에서 NULL 이 나타나면 컴파일 단계에서 오류가 발생한다.",
      "T2 의 행 수만큼 카티션 곱 형태로 반환된다."
    ],
    "correctIndex": 1,
    "explanation": "복수컬럼 IN 비교 `(A, B) IN (SELECT A, B FROM T2)` 는 각 튜플 단위로 동등 비교한다. T2 의 B 가 NULL 인 행과 비교할 때 `B = NULL` 부분이 UNKNOWN 으로 평가되어 매칭에 기여하지 못한다. T1 의 행이 T2 의 정상 (A, B) 튜플과 매칭되면 결과에 포함되지만, NULL 이 섞인 T2 행은 매칭 후보에서 제외된다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 26,
    "title": "아래 EMP 테이블에 RANK 와 DENSE_RANK 를 적용했을 때 마지막 행의 두 값으로 옳은 것은?",
    "options": [
      "RANK 4, DENSE_RANK 3",
      "RANK 3, DENSE_RANK 3",
      "RANK 4, DENSE_RANK 4",
    ...
    "correctIndex": 0,
    "explanation": "SAL 내림차순으로 500, 400, 400, 300 이 되어 400 두 건이 공동 2 위가 된다. RANK 는 공동 순위가 차지한 자리를 건너뛰어 다음을 4 위로 매기고, DENSE_RANK 는 건너뛰지 않아 3 위로 매긴다. 따라서 마지막 행(300)은 RANK 4, DENSE_RANK 3 이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "INTERSECT 는 두 집합에 모두 존재하는 행만 남기는 교집합 연산이다. T1 과 T2 에 공통으로 있는 값은 B 와 C 이므로 두 건이 반환되며 결과는 기본적으로 오름차순 정렬된다. 합집합이면 A·B·C·D, 차집합(MINUS)이면 A 가 나온다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 50,
    "title": "아래 7건 데이터에 NTILE(3) 을 적용한 그룹별 건수에서 빈칸 (ㄱ), (ㄴ) 으로 옳은 것은?",
    "options": [
      "ㄱ = 1, ㄴ = 3",
      "ㄱ = 1, ㄴ = 2",
      "ㄱ = 2, ㄴ = 3",
    ...
    "correctIndex": 0,
    "explanation": "NTILE(3) 은 7건을 세 그룹으로 나눌 때 앞 그룹에 한 건을 더 배정해 그룹별 3, 2, 2 건으로 구성한다. 따라서 (ㄱ) 은 첫 그룹 번호 1, (ㄴ) 은 첫 그룹의 건수 3 이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
        "caption": "원본 T 테이블 (총 7건)",
        "headers": [
          "VAL"
        ],
        "rows": [
