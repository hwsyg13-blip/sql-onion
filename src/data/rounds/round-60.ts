...
export const ROUND_60: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "ID가 일치하지 않으므로 NOT MATCHED 분기가 실행되어 SRC 행이 TGT에 INSERT된다. INSERT 절에서 V1·V2·V3 모두 100을 명시했으므로 세 컬럼 값은 모두 100이 된다. MATCHED 분기가 실행되었다면 V3는 갱신 대상이 아니어서 NULL이 남았겠지만, 본 문제에서는 INSERT 분기가 동작한다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "부서 내 동률을 같은 순위로 매기되 다음 순위를 건너뛰지 않는 DENSE_RANK가 '상위 N명' 조회에 가장 적합하다. ② ROWNUM은 분석 함수가 아니며 PARTITION BY와 함께 쓸 수 없고, ③ NTILE은 등분 그룹을 매기는 용도이며, ④ CUME_DIST는 누적 분포 비율(0~1)을 반환하므로 상위 3명 필터링에 부적합하다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
