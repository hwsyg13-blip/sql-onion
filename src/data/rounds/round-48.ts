...
export const ROUND_48: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "테이블 구조는 배우(배우번호, 배우명, 성별)·영화(영화번호, 영화명, 제작년도)·출연(배우번호, 영화번호, 출연료)이므로 영화명은 영화 테이블, 배우명은 배우 테이블, 출연료는 출연 테이블에서 가져와야 합니다. ①·②·③은 컬럼 소유 테이블을 잘못 짚거나 조건 부등호를 잘못 쓴 형태입니다. ④번은 출연·영화·배우 세 테이블을 출연.영화번호 = 영화.영화번호 AND 출연.배우번호 = 배우.배우번호로 정확히 조인하고 출연료 >= 8888 조건까지 올바르게 기술되어 있어 정답입니다.",
    "_source": "authored",
    "references": [
      {
        "type": "ascii",
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "CEIL 은 올림 함수이며 4.1 의 올림은 5 가 되어 가장 큰 값이다.",
    "_source": "authored"
  },
  {
    "id": 10617,
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "데이터(SAL): 2000, 1500, 1500, 1500을 SAL 내림차순으로 정렬했다고 보면, DENSE_RANK는 1, 2, 2, 2가 되고 RANK도 1, 2, 2, 2가 됩니다(동일 순위 뒤 RANK는 다음 번호를 건너뛰지만 여기서는 동일 순위가 마지막까지 이어져 결과가 같음). FIRST_VALUE는 윈도우의 첫 번째 행 값을 모든 행에 동일하게 반환하므로 정렬 첫 값인 2000이 모든 행에 나옵니다. ①·②는 부분적으로는 옳지만 \"FIRST_VALUE 가 모든 행에 2000\"이라는 결정적 식별 조건은 ③번이므로 정답은 ③번입니다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "조건(A >= B)을 만족하는 행이 총 10건이고 각 행의 A 값이 10이라고 가정하면 SUM(A) = 10 × 10 = 100입니다. 보기 ①(10)은 한 행 값, ②(40)는 4건 × 10, ④(NULL)는 빈 결과를 의미하므로 모두 의도와 다릅니다. 정답은 ③번입니다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
