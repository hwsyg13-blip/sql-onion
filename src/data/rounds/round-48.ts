...
export const ROUND_48: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "정렬 후 첫 행 값을 기준으로 비교하면 됩니다. SAL 내림차순 정렬 시 1순위가 명확하다면 RANK·DENSE_RANK는 모두 1을 부여합니다. 그러나 동순위 발생 시 RANK는 다음 순위를 건너뛰지만 DENSE_RANK는 연속된 다음 순위를 부여합니다. FIRST_VALUE는 정렬된 윈도우의 첫 행 값을 모든 행에 동일하게 반환하므로, 데이터에 관계없이 첫 행이 결정되어 있으면 모든 행에서 같은 값이 나옵니다. 정답은 ③번입니다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "본 문항은 원본 기출에서 표 데이터 일부가 유실되어 메타 형태로 정리된 케이스입니다. 핵심은 NTILE(N) 함수가 정렬된 데이터를 N개 그룹으로 최대한 균등하게 나누되, 나누어 떨어지지 않으면 앞쪽 그룹부터 한 건씩 더 배정한다는 점입니다. 데이터가 4행 2컬럼 형태에서 NTILE(3)을 적용하면 (1, 1), (2, 1), (3, 2), (4, 3) 같은 분포가 되어 정답은 ④번입니다.",
    "_source": "authored"
  },
  {
    "id": 10644,
