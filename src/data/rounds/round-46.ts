...
export const ROUND_46: QuizQuestion[] = [
  {
    ...
    "correctIndex": 2,
    "explanation": "데이터 모델링 단계는 개념 → 논리 → 물리 순으로 진행된다. ③ \"주제영역을 설계하고 업무 전반의 범위를 정의\" 는 모델링 가장 앞 단계인 개념 데이터 모델링의 산출물이므로 논리 모델링 설명으로 적절하지 않아 정답이 된다. ① 엔터티·속성·관계 도출, ② 식별자 확정 + 정규화 수행, ④ 슈퍼·서브 타입 분리·통합은 모두 논리 모델링의 주요 활동이다.",
    "_source": "authored"
  },
  {
    "id": 10701,
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "ERD 표기 `마더보드 ||--o| 컴퓨터` 의 카디널리티 해석 — 컴퓨터 쪽 끝의 `o|` 는 \"0 또는 1, 선택\" 을 의미한다. 즉 마더보드 한 개에 대해 컴퓨터는 \"있을 수도 있고 없을 수도 있다\" → 마더보드는 컴퓨터에 \"때때로\" (선택적) 포함되므로 ② 가 정답. ① 은 \"반드시\" 라는 단정이라 선택사양과 어긋나고, ③ 은 1:N 의 N 측이 없으며, ④ 는 다대다(\\\\}\\\\o--o{\\\\) 표기가 아니라 모두 옳지 않다.",
    "_source": "authored",
    "references": [
      {
        "type": "erd",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "결과 표를 보면 (매장ID, 매장명, 년도) 세부 집계 + (매장ID, 년도) 매장명 단위 소계 + (년도) 전체 소계 가 함께 나타난다. ROLLUP(매장ID, 매장명) 은 왼쪽부터 단계적으로 키를 줄여 (매장ID, 매장명) → (매장ID) → () 세 그룹을 만들고, 그 각각에 \"년도\" 차원이 곱해져 출력 결과와 정확히 일치한다. ① GROUPING SETS 는 (매장ID) 와 (매장ID, 매장명) 만 만들어 () 전체 소계가 빠지고, ③ CUBE 는 모든 부분집합을 만들어 결과보다 행이 더 많고, ④ 일반 GROUP BY 는 소계 자체가 없다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
        "caption": "EMP 매출 테이블 (입력)",
        "headers": [
          "매장ID",
          "매장명",
          "년도",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 19,
    "title": "아래 세 테이블에 대한 SQL 의 결과 행 수로 옳은 것은?",
    "options": [
      "3건",
      "4건",
      "5건",
    ...
    "correctIndex": 1,
    "explanation": "T1 UNION T2 = {1, 2, 3, 4, 5} (UNION 은 중복 제거). 거기서 T3 = {2} 를 MINUS 하면 {1, 3, 4, 5} 로 총 4 건이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 21,
    "title": "아래 데이터에 네 윈도우 함수를 적용했을 때, 마지막 행의 값이 나머지와 다른 것은?",
    "options": [
      "RANK() OVER (ORDER BY VAL)",
      "DENSE_RANK() OVER (ORDER BY VAL)",
      "ROW_NUMBER() OVER (ORDER BY VAL)",
      "NTILE(5) OVER (ORDER BY VAL)"
    ],
    "correctIndex": 1,
    "explanation": "VAL 이 20 인 행이 두 개라 동률이 생긴다. RANK 는 1, 2, 2, 4, 5 로 동률 다음 순위를 건너뛰고 DENSE_RANK 는 1, 2, 2, 3, 4 로 건너뛰지 않는다. ROW_NUMBER 는 동률과 무관하게 1, 2, 3, 4, 5 이고 NTILE(5) 도 다섯 그룹으로 나누어 1, 2, 3, 4, 5 가 된다. 마지막 행의 값이 4 인 DENSE_RANK 만 나머지(5)와 다르다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "options": [
      "전체 직원 중 급여가 가장 큰 1 명만 단일 행으로 반환한다.",
      "부서별 최상위 급여자를 모두 묶어 반환한다.",
      "WHERE 조건이 항상 참이 되어 모든 직원을 그대로 반환한다.",
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "ERD 표기 `지점 |o--o{ 고객` — 지점 측 끝의 `|o` 는 \"0 또는 1, 선택\" (고객은 지점에 소속될 수도 있고 안 될 수도 있다), 고객 측 끝의 `o{` 는 \"0..N, 선택\" (지점에 소속된 고객이 0명일 수도 있다) 을 의미한다. 양쪽 모두 선택사양이므로 ① \"지점이 최소 한 명의 고객을 반드시 가진다\" 는 거짓 (0명 가능), ② \"고객은 반드시 하나의 지점에 소속된다\" 도 거짓 (지점 없는 고객 가능), ④ 다대다도 아니다. ③ 처럼 지점에 소속되지 않은 고객이 존재할 수 있으므로 전체 고객 조회 시 OUTER JOIN 또는 별도 조회가 필요하다.",
    "_source": "authored",
    "references": [
      {
        "type": "erd",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 39,
    "title": "아래 두 테이블에 대해 NOT IN 서브쿼리를 사용한 SQL 의 결과 행 수로 옳은 것은? (T2.COL2 에 NULL 포함)",
    "options": [
      "T1의 모든 행",
    ...
    "correctIndex": 2,
    "explanation": "`NOT IN` 목록에 NULL 이 포함되면 `COL1 <> NULL` 평가가 UNKNOWN 이 되어 어떤 행도 조건을 TRUE 로 만족시키지 못한다. 따라서 결과는 0 건이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
