...
export const ROUND_54: QuizQuestion[] = [
  ...
  },
  {
    ...
    "options": [
      "IE 표기법은 관계선 끝의 까마귀발(crow’s foot) 로 다(多) 카디널리티를 표시한다.",
      "Barker 표기법은 관계선의 실선과 점선으로 식별 관계와 비식별 관계를 구분한다.",
      "IE 표기법에서 관계선의 실선은 식별 관계를, 점선은 비식별 관계를 나타낸다.",
      "Barker 표기법은 관계선의 실선과 점선으로 필수 참여와 선택 참여를 구분한다."
    ],
    "correctIndex": 1,
    "explanation": "Barker(오라클) 표기법에서 관계선의 실선·점선은 필수 참여와 선택 참여를 나타내며, 식별 관계는 관계선 위의 UID bar 로 표시한다. 실선·점선으로 식별·비식별 관계를 구분하는 것은 IE 표기법이므로 ② 가 적절하지 않다.",
    "_source": "authored"
  },
  {
    "id": 10309,
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "뷰는 조회 속도를 높이려고 쓰는 객체가 아니라 논리적 독립성, 질의 단순화, 보안(접근 제어)을 위해 사용한다. 뷰는 실제 데이터를 저장하지 않고 다른 테이블을 참조하며, 기본 테이블에 컬럼이 추가되어도 뷰 정의를 바꾸지 않으면 응용 프로그램에 영향이 없다. 따라서 적절하지 않은 것은 ①이다.",
    "_source": "authored"
  },
  {
    "id": 10312,
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "정규식 `^[0-9]{3}-[0-9]{3,4}-[0-9]{4}$` 는 \"숫자 정확히 3자리 - 숫자 3 또는 4자리 - 숫자 정확히 4자리\" 형식이다.\n\n① 010-555-6789 (3-3-4): 가운데 그룹이 3자리지만 `[0-9]{3,4}` 가 3·4자리 모두 허용하므로 매칭 ✓\n② 011-1234-5678 (3-4-4): 모든 그룹 길이 부합 → 매칭 ✓\n④ 070-9999-1234 (3-4-4): 매칭 ✓\n③ 02-1234-5678 (2-4-4): 첫 그룹이 2자리 (서울 지역번호 02) 라 `[0-9]{3}` (정확히 3자리) 조건에 맞지 않음 → 매칭 실패 ✗\n\n주의: `[0-9]{3,4}` 는 가운데 그룹에만 적용되며, 첫·끝 그룹은 각각 정확한 자릿수를 요구한다. 서울 지역번호 02 처럼 실제로 사용되는 형식이라도 정규식 조건을 위반하면 결과에서 제외된다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "options": [
      "SELECT STADIUM.NAME FROM HR.STADIUM, SPORT.STADIUM;",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 21,
    "title": "아래 두 테이블을 NATURAL JOIN 한 결과로 옳은 것은?",
    "options": [
      "3건",
      "2건",
      "1건",
    ...
    "correctIndex": 0,
    "explanation": "NATURAL JOIN 은 두 테이블에서 동일한 이름을 가진 모든 컬럼을 자동으로 조인 조건으로 사용한다. T1·T2 에서 KEYA·KEYB 는 이름이 다르므로 조인 조건에서 제외되고, 동일명 컬럼인 COL1·COL2 만 매칭 대상이 된다. (A,가) 그룹: T1 의 (1,A,가)/(2,A,가) 2행 × T2 의 (1,A,가) 1행 = 2건, (C,다) 그룹: T1 의 (5,C,다) 1행 × T2 의 (3,C,다) 1행 = 1건. 합계 3건. 결과 테이블의 공통 컬럼은 한 번씩만 나타나며 KEYA·KEYB 는 그대로 보존된다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 30,
    "title": "아래 결과(COL1 에만 NULL 이 있는 집계) 를 출력하기 위한 GROUPING SETS 로 적절한 것은?",
    "options": [
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "값 범위 기준 RANGE 윈도우는 정렬 기준 컬럼이 필요하므로 ORDER BY 가 있어야 한다. ② 는 ORDER BY 가 없어 성립하지 않고 ④ 가 연봉 오름차순 정렬(ORDER BY SALARY)과 RANGE 범위를 함께 지정하여 -100~200 범위의 건수를 집계한다.",
    "_source": "authored"
  },
  {
    "id": 10332,
  ...
  },
  {
    ...
    "options": [
      "SELECT * FROM T WHERE ROWNUM > 0;",
      "SELECT * FROM T WHERE ROWNUM = 2;",
      "SELECT * FROM T WHERE ROWNUM < 3;",
      "SELECT * FROM T WHERE ROWNUM <= 2;"
    ],
    "correctIndex": 1,
    "explanation": "ROWNUM 은 1부터 순차 부여되므로 첫 행이 ROWNUM=1 을 받고 탈락하면 다음 행에도 1 이 할당되어 2에 도달하지 못한다. 따라서 ROWNUM = 2 는 0건을 반환하고, ROWNUM > 0, ROWNUM < 3, ROWNUM <= 2 는 모두 2건을 반환한다.",
    "_source": "authored"
  },
  {
    "id": 10336,
  ...
  },
  {
    ...
    "options": [
      ...
      "DELETE FROM 부서;",
      "ALTER TABLE 부서 DROP COLUMN 부서명;"
    ],
    "correctIndex": 1,
