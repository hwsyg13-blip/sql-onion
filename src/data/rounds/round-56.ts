...
export const ROUND_56: QuizQuestion[] = [
  ...
  },
  {
    ...
    "subject": "1과목",
    "number": 5,
    "title": "아래 [주문] 엔터티의 속성 중 파생 속성에 해당하는 것은?",
    "options": [
      "주문번호",
      "주문일자",
      "주문수량",
    ...
    "correctIndex": 3,
    "explanation": "파생 속성은 다른 속성으로부터 계산되어 만들어지는 속성이다. 총주문금액은 단가 × 주문수량으로 도출되므로 파생 속성이고, 나머지는 업무에서 직접 입력·부여되는 기본 속성이다. 파생 속성은 정합성 관리 부담이 있어 가급적 최소화한다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 31,
    "title": "아래 상황에서 지켜지지 않은 트랜잭션의 특성은?",
    "options": [
      "원자성",
      "일관성",
      "고립성",
    ...
    "correctIndex": 2,
    "explanation": "고립성(Isolation)은 동시에 실행되는 트랜잭션이 서로의 중간 상태를 보지 못하도록 격리하는 성질이다. 커밋되지 않은 다른 트랜잭션의 값을 읽은 것은 고립성 위반에 해당한다. 원자성은 전부 반영되거나 전부 취소되는 성질, 지속성은 커밋된 결과가 영구 보존되는 성질이다.",
    "_source": "authored",
    "references": [
      {
        "type": "text",
        "content": "A 트랜잭션이 계좌 잔액을 읽어 계산하는 도중, 아직 커밋되지 않은 B 트랜잭션의 수정 값을 함께 읽어 잘못된 금액을 산출했다."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 42,
    "title": "아래 부서·사원 테이블에 대한 쿼리 실행 결과에 대한 설명으로 옳은 것은?",
    "options": [
      "등급이 2 이상인 모든 사원이 출력된다.",
      "부서에 소속되지 않은 사원과 등급이 2 미만인 사원은 결과에서 제외된다.",
      "부서 정보가 없는 사원도 결과에 포함된다.",
