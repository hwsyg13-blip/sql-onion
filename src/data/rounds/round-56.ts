...
export const ROUND_56: QuizQuestion[] = [
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 34,
    "title": "아래 EMP 테이블에서 사원의 관리자와 차상위 관리자를 함께 출력하는 쿼리로 가장 적절한 것은?",
    "options": [
    ...
    "correctIndex": 2,
    "explanation": "셀프 조인으로 \"A 가 관리하는 하위 사원 B\" 를 매칭하려면 `A.EMP_ID = B.MGR_ID` 조건이 필요. LEFT OUTER JOIN 으로 매칭되지 않는 A(관리하는 부하 없는 사원) 도 결과에 포함. ① 자기 자신과 매칭, ② 는 \"A 의 상사 = B\" 매칭, ④ 자기 자신 LEFT OUTER 라 모두 의도와 다름.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
