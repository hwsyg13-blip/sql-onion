...
export const ROUND_45: QuizQuestion[] = [
  ...
  },
  {
    ...
    "references": [
      {
        "type": "text",
        "content": "우리은행은 예금분류(일반예금·특별예금 등)의 원금, 예치기간, 이자율을 관리하고자 한다. 또한 원금에 대한 이자율을 적용하여 계산된 이자를 별도 속성으로 관리하며, 일반예금·특별예금 등에 대해서는 코드를 부여(예: 01-일반예금, 02-특별예금)한다."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "관계차수는 두 엔터티 간 관계에 참여하는 인스턴스 수의 대응 관계를 나타내며, `|`(단일성)·`o`(선택성)·`<`(까마귀발, 다수) 기호 조합으로 1:1, 1:M, M:N 등을 표기한다. 예: `[A] ─|─|─ [B]`(1:1), `[A] ─|─< [B]`(1:N), `[A] ─|─o< [B]`(1:0·1·N).",
    "_source": "authored"
  },
  {
    "id": 10758,
  ...
  },
  {
    ...
    "references": [
      {
        "type": "text",
        "content": "A 단위 소계와 (A, B) 단위 합계, 그리고 전체 합계가 함께 도출되어야 한다."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "references": [
      ...
      {
        "type": "sql",
        "code": "SELECT AVG(NVL(C1, 0)) FROM T;"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 31,
    "title": "아래 네 테이블에 대한 SQL 의 결과 행 수로 옳은 것은?",
    "options": [
      "1",
      "2",
      "3",
    ...
    "correctIndex": 0,
    "explanation": "`T2.COL1(+) = T1.COL1` 은 T1 LEFT OUTER JOIN T2, `T3.COL1(+) = T2.COL1` 은 그 결과에 T3 LEFT OUTER JOIN, 마지막 `T4.COL1 = T3.COL1` 은 INNER JOIN. 마지막이 INNER JOIN 이라 T3·T4 양쪽에 모두 있는 키만 살아남고, 그 키가 다시 T1·T2 와 매칭되는 행만 남는다. 네 테이블 공통 키는 1 한 건이라 결과는 1 행.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 33,
    "title": "아래 두 테이블에 대해 NOT IN 서브쿼리를 사용한 SQL 의 결과로 옳은 것은? (T3 의 N1 에 NULL 포함)",
    "options": [
      "T1의 모든 행",
      "T3에 없는 A.N1에 해당하는 행",
      "0건",
    ...
    "correctIndex": 2,
    "explanation": "`NOT IN` 목록에 NULL 이 포함되면 `A.N1 <> NULL` 평가가 UNKNOWN 이 되어 어떤 행에서도 조건이 TRUE 로 평가되지 않는다. 따라서 매칭되지 않는 행조차 결과에서 제외되어 0 건이 반환된다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "references": [
      ...
      {
        "type": "sql",
        "code": "SELECT T1.ID, T1.NAME, T2.TYPE\nFROM   T1 LEFT OUTER JOIN T2\nON     T1.ID = T2.ID AND T2.TYPE = 'X'\nWHERE  T2.TYPE = 'X';"
      }
    ]
  },
  {
    ...
    "references": [
      {
        "type": "sql",
        "code": "SELECT /*+ INDEX(E EMP_DEPT_IDX) */ E.*\nFROM   EMP E\nWHERE  E.DEPT_NO = 10;"
      }
    ]
  },
  {
