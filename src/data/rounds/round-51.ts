...
export const ROUND_51: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "표의 모든 행은 COL1 = 'CPG' 이거나 COL2 ≤ 10 이라 `COL1 <> 'CPG' AND COL2 > 10` 두 조건을 동시에 만족하는 행이 없다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "SUBSTR('...의 마...', 4, 3) 은 4번째 문자부터 3글자를 추출 → '의 마'. ROUND(109, 1) 은 소수 첫째 자리까지 반올림하지만 정수 109 는 변환할 소수가 없으므로 109 가 그대로 유지된다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "SAL 이 같은 행이 두 개 있을 때 RANK 는 1, 2, 2, 4 로 동률 다음 순위를 건너뛰고 DENSE_RANK 는 1, 2, 2, 3 으로 건너뛰지 않는다. ROW_NUMBER 는 동률과 무관하게 1, 2, 3, 4 를 부여하고 NTILE(4) 도 네 그룹으로 나누어 1, 2, 3, 4 가 된다. 마지막 행의 값이 3 인 DENSE_RANK 만 나머지(4)와 다르다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 36,
    "title": "Oracle 에서 매출 상위 1·2위를 조회하는 SQL 로 옳은 것은?",
    "options": [
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "데이터가 NULL 을 포함하지 않는 전제에서 세 방식 모두 같은 결과를 낸다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
