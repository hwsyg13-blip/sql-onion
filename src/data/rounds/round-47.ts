...
export const ROUND_47: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "MERGE 는 ON 조건으로 대상(A)과 원본(B)을 매칭해, 매칭되면 UPDATE, 매칭되지 않으면 INSERT 를 수행한다. ID 1 은 A 와 B 에 모두 있어 UPDATE 로 V=100 이 되고, ID 2 는 B 에 없어 변경되지 않고 V=0 으로 남으며, ID 3 은 A 에 없어 INSERT 로 V=100 이 추가된다. 따라서 결과 V 는 100, 0, 100 이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
