...
export const ROUND_55: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "DENSE_RANK 는 동일 값 그룹에 같은 순위를 부여하고 다음 순위를 \"건너뛰지 않고\" 1 씩 증가시킨다. PARTITION BY DEPT 로 부서별 독립 순위. DEPT=10 (3 행) — SAL DESC 정렬 시 3000 (1위) · 3000 (1위 동률) · 2500 (다음 = 2위, RANK 였다면 3위로 건너뛰었을 것). DEPT=20 (1 행) — 5000 단독 1위. 테이블 행 순서대로 RNK 는 1, 1, 2, 1 → ②.\n\n각 보기 시나리오: ① 1, 2, 2, 1 — ROW_NUMBER 처럼 첫 두 동률을 1·2 로 부여 후 2 위에 머무는 비표준 동작. ③ 1, 2, 3, 1 — ROW_NUMBER (PARTITION BY DEPT) 결과 — 동률을 무시하고 고유 순번. ④ 1, 1, 3, 1 — RANK (동률 후 다음 순위 건너뛰는 표준) 결과.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 37,
    "title": "Oracle 의 ORDER BY 절에서 NULL 값이 포함된 경우의 기본 정렬 동작으로 옳은 것은?",
    "options": [
    ...
    "correctIndex": 1,
    "explanation": "Oracle 은 NULL 을 정렬 시 \"가장 큰 값\" 으로 취급한다. 따라서 ASC (오름차순) 에서는 NULL 이 마지막에 위치 (NULLS LAST), DESC (내림차순) 에서는 NULL 이 가장 먼저 위치 (NULLS FIRST) 한다 → 정답 ②.\n\n참고 — DBMS 별 기본 동작은 다르다:\n- Oracle / PostgreSQL: NULL 을 큰 값으로 취급 → ASC NULLS LAST, DESC NULLS FIRST (옵션 ②)\n- SQL Server / MySQL: NULL 을 작은 값으로 취급 → ASC NULLS FIRST, DESC NULLS LAST (옵션 ①)\n\n어느 DBMS 든 ORDER BY 컬럼 뒤에 NULLS FIRST 또는 NULLS LAST 를 명시하면 기본 동작과 무관하게 강제할 수 있다 (단, MySQL 은 NULLS FIRST/LAST 키워드 미지원).",
    "_source": "authored"
  },
  {
    "id": 10287,
