...
export const ROUND_55: QuizQuestion[] = [
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "원자성(Atomicity)은 트랜잭션의 모든 연산이 모두 수행되거나 전혀 수행되지 않아야 한다는 특성이다.",
    "_source": "authored"
  },
  {
    "id": 10257,
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "데이터 모델의 구성 요소는 엔터티(집합) → 인스턴스(개별 행) → 속성(컬럼) → 속성값(셀 값) 순서의 포함 관계로 정의된다.",
    "_source": "authored",
    "references": [
      {
        "type": "text",
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "DML(Data Manipulation Language)은 데이터 조작어로 INSERT·UPDATE·DELETE·SELECT가 해당된다. CREATE/ALTER/DROP은 DDL, GRANT/REVOKE는 DCL, COMMIT/ROLLBACK은 TCL이다.",
    "_source": "authored"
  },
  {
    "id": 10262,
    ...
    "subject": "2과목",
    "number": 13,
    "title": "아래 SQL 의 RNK 컬럼 결과로 적절한 것은?",
    "options": [
      "1, 2, 2, 1",
      "1, 1, 2, 1",
      "1, 2, 3, 1",
    ...
    "correctIndex": 1,
    "explanation": "DENSE_RANK 는 같은 값에 같은 순위를 주되 다음 순위를 건너뛰지 않고 1 씩 올린다. PARTITION BY DEPT 로 부서별 순위를 따로 매기므로 DEPT 10 은 3000, 3000, 2500 에 대해 1, 1, 2 가 되고 DEPT 20 은 단독 행이라 1 이 된다. 테이블 행 순서대로 읽으면 1, 1, 2, 1 이다. RANK 였다면 세 번째가 3 으로 건너뛰었을 것이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "`GROUP BY ROLLUP(COL1), COL1` 은 `GROUP BY COL1 UNION ALL GROUP BY COL1` 과 동일하다 (ROLLUP 의 소계가 COL1 별 그룹과 결합됨). 그룹 = {A, B, C} 3 개 × 2 회 = 6 행이 산출되고 외부 `COUNT(*)` 가 6 을 반환한다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "Oracle 식별자 규칙상 테이블명은 영문자로 시작해야 하며 숫자로 시작할 수 없다. VARCHAR2(20)·NUMBER(크기 생략 허용)·세미콜론은 모두 정상 구문이다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 19,
    "title": "아래 T 테이블에 대한 SQL 의 결과로 적절한 것은?",
    "options": [
    ...
    "correctIndex": 2,
    "explanation": "`SUM(C1)` 과 `SUM(C2)` 는 각각 NULL 을 자동 제외하고 집계한 후, 두 집계 값을 더한다. 본 데이터에서 `SUM(C1)` = 100+200+300 = 600, `SUM(C2)` = 10+30 = 40, 합계 640. NVL 은 결과가 NULL 인 경우만 0 으로 치환 (이 케이스는 NULL 아니라 그대로 640).",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 21,
    "title": "아래 네 테이블에 대한 SQL 의 결과 행 수는?",
    "options": [
      "1 건",
      "3 건",
      "0 건",
    ...
    "correctIndex": 0,
    "explanation": "마지막 `INNER JOIN T4` 가 이전까지의 OUTER JOIN 결과 중 T4 와 매칭되는 행만 남긴다. 네 테이블 공통 ID 는 1 한 건이라 최종 1 행이 반환된다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 24,
    "title": "아래 EMP 테이블에 대한 계층형 질의에서 순방향(부모 → 자식) 전개가 되도록 CONNECT BY 절을 완성하는 것은?",
    "options": [
      "PRIOR 사원번호 = 관리자번호",
      "PRIOR 관리자번호 = 사원번호",
      "사원번호 = 관리자번호",
    ...
    "correctIndex": 0,
    "explanation": "순방향(부모→자식) 전개에서는 \"이전(부모) 행의 사원번호가 다음(자식) 행의 관리자번호와 일치\" 해야 하므로 `PRIOR 사원번호 = 관리자번호`. ② 는 자식→부모 역방향, ③ 은 PRIOR 없어 모든 행이 단일 그룹, ④ 는 PRIOR 가 양쪽이라 자기 참조라 무한.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "ORDER BY 절은 SELECT 절이 평가된 뒤에 수행되므로 SELECT 절에서 부여한 별칭을 그대로 참조할 수 있다. 따라서 별칭을 참조할 수 없다는 ③ 이 적절하지 않다. ② 는 같은 이유로 정상 동작하고, ① 은 5 × 3 = 15 회로 옳으며, ④ DISTINCT 는 중복 행을 하나로 묶는다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
      ...
      {
        "type": "sql",
        "code": "SELECT   회원ID AS ALIAS1,\n         TRUNC(로그인일시) AS ALIAS2,\n         COUNT(*) AS 로그인횟수\nFROM     로그인이력\nGROUP BY 회원ID, TRUNC(로그인일시)\nORDER BY ALIAS1, ALIAS2;"
      }
    ]
  },
  {
    ...
    "correctIndex": 2,
    "explanation": "컬럼 별칭은 컬럼 표현식 뒤에만 쓸 수 있고 앞에는 붙일 수 없으므로 ③ 이 적절하지 않다. AS 는 생략할 수 있고, 공백이나 특수문자가 들어간 별칭은 큰따옴표로 감싸며, ORDER BY 는 SELECT 이후에 수행되어 별칭을 참조할 수 있다.",
    "_source": "authored"
  },
  {
    "id": 10277,
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "UNION·UNION ALL 등 집합 연산 결과의 컬럼명은 첫 번째 SELECT 절의 별칭을 따른다. 두 번째 SELECT 가 같은 컬럼에 다른 별칭(BBA, AAB)을 붙여도 무시되므로 결과 컬럼명은 AAB, BBA 가 된다. ② 는 두 번째 SELECT 의 별칭이 적용된다고 본 경우, ③ 은 별칭이 모두 무시된다고 본 경우, ④ 는 양쪽 별칭이 다르면 오류가 난다고 본 경우이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "ROLLBACK TO SAVEPOINT A 는 저장점 A 이후의 변경만 취소하고 그 이전의 변경은 유지한다. 따라서 A 이전에 실행된 (가) 는 살아남고 A 이후의 (나)·(다) 는 취소된다. 이후 실행된 (라)·(마) 는 롤백의 영향을 받지 않으므로 COMMIT 시점에 영구 반영되는 것은 (가), (라), (마) 세 개다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "Oracle 은 오름차순 정렬에서 NULL 을 가장 뒤에, 내림차순 정렬에서 가장 앞에 배치한다. NULL 을 가장 큰 값처럼 취급하기 때문이며, 이 순서를 바꾸려면 NULLS FIRST 나 NULLS LAST 를 명시해야 한다.",
    "_source": "authored"
  },
  {
    "id": 10287,
  ...
  },
  {
    ...
    "options": [
      ...
      "(A INTERSECT B) UNION ALL (A MINUS B)",
      "(A UNION B) MINUS (A INTERSECT B)"
    ],
    "correctIndex": 0,
    "explanation": "(A UNION B)는 양쪽 모두에 있는 행을 한 번씩 포함하는 합집합. (A INTERSECT B)는 양쪽 공통 행. UNION ALL은 두 결과를 그대로 이어 붙이므로, (A UNION B) UNION ALL (A INTERSECT B)는 A UNION ALL B와 같은 행 수를 가진다(공통 행이 두 번 등장하는 효과).",
    "_source": "authored"
  },
  {
    "id": 10291,
    ...
    "subject": "2과목",
    "number": 42,
    "title": "아래 T 테이블에 대한 세 SQL 의 결과로 옳은 것은?",
    "options": [
      "0, 0, 0",
      "0, ERROR, NULL",
      "NULL, ERROR, 0",
    ...
    "correctIndex": 1,
    "explanation": "(1) `0 / 300` = 0. (2) `300 / 0` 은 0 으로 나누기 오류 ORA-01476 발생. (3) `100 / NULL` 은 NULL 연산 결과로 NULL 반환.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 1,
    "explanation": "기존 테이블에 컬럼을 추가할 때는 ALTER TABLE ... ADD (컬럼명 데이터타입) 구문을 사용한다. CREATE는 신규 생성, UPDATE는 데이터 수정, INSERT는 행 추가용이다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 50,
    "title": "아래 EMP 테이블에 대한 SQL 에 대한 설명으로 적절하지 않은 것은?",
    "options": [
    ...
    "correctIndex": 3,
    "explanation": "`IN` 절은 다중 행 서브쿼리를 허용하므로 EMPNM 이 'JOHN' 인 직원이 두 명이어도 서브쿼리는 두 행의 DEPT 를 반환하고 외부 IN 비교가 정상 동작한다. ④ \"두 명일 경우 오류 발생\" 은 잘못된 설명.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
