...
export const ROUND_52: QuizQuestion[] = [
  ...
  },
  {
    ...
    "subject": "1과목",
    "number": 3,
    "title": "아래 상황에서 보장되는 데이터 독립성과, 변경이 일어난 스키마를 올바르게 짝지은 것은?",
    "options": [
      "논리적 독립성 — 개념 스키마 변경",
      "물리적 독립성 — 내부 스키마 변경",
      "논리적 독립성 — 내부 스키마 변경",
    ...
    "correctIndex": 1,
    "explanation": "파티션과 인덱스는 디스크에 데이터를 어떻게 저장할지를 다루는 내부 스키마의 영역이다. 내부 스키마가 바뀌어도 개념·외부 스키마가 영향을 받지 않는 성질을 물리적 독립성이라 한다. 반대로 개념 스키마(테이블 구조)가 바뀌어도 응용 프로그램이 영향을 받지 않는 것은 논리적 독립성이다.",
    "_source": "authored",
    "references": [
      {
        "type": "text",
        "content": "운영 중인 데이터베이스에서 ORDERS 테이블을 월별 파티션으로 나누고 인덱스를 새로 추가했다. 이 작업 뒤에도 기존 응용 프로그램의 SQL 은 전혀 수정하지 않았다."
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "references": [
      {
        "type": "erd",
        "caption": "교수·학생·과목 ERD (교수-학생, 학생-과목은 각각 M:N 관계)",
        "mermaid": "erDiagram\n    교수 }o--o{ 학생 : \"담당\"\n    학생 }o--o{ 과목 : \"이수\""
      }
    ]
  },
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "루트(START WITH 매니저 IS NULL)에서 시작해 부모 행의 사원 값과 자식 행의 매니저 값이 일치하는 행을 따라 내려가는 순방향 전개이므로 PRIOR 는 사원 앞(ㄱ)에 온다. 즉 CONNECT BY PRIOR 사원 = 매니저 형태이다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 19,
    "title": "아래 SQL 의 RNK 컬럼 값을 위에서부터 순서대로 나열한 것은?",
    "options": [
      "1, 2, 1, 2",
      "1, 1, 1, 1",
      "1, 2, 3, 4",
    ...
    "correctIndex": 0,
    "explanation": "PARTITION BY DEPT 는 부서별로 순위를 따로 매긴다. DEPT 10 안에서 300 이 1 위, 200 이 2 위이고 DEPT 20 안에서도 500 이 1 위, 400 이 2 위가 되어 순위가 부서마다 1 부터 다시 시작한다. PARTITION BY 가 없었다면 전체를 한 묶음으로 보아 1, 2, 3, 4 가 된다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
  ...
  },
  {
    ...
    "correctIndex": 0,
    "explanation": "INSERT 는 DML 이므로 DCL 과 짝지어지지 않는다. SELECT 도 SQLD 에서는 DML(데이터 조회)로 분류한다.",
    "_source": "authored"
  },
  {
    "id": 10420,
  ...
  },
  {
    ...
    "subject": "2과목",
    "number": 24,
    "title": "아래 SQL 이 오류를 일으키는 이유로 가장 적절한 것은?",
    "options": [
      "WHERE 절은 GROUP BY 보다 먼저 평가되어 집계 함수를 쓸 수 없다",
      "GROUP BY 절에 집계 함수를 쓸 수 없다",
      "SELECT 절의 별칭은 WHERE 절에서 참조할 수 없다",
      "ORDER BY 절은 별칭을 참조할 수 없다"
    ],
    "correctIndex": 0,
    "explanation": "SQL 은 FROM → WHERE → GROUP BY → HAVING → SELECT → ORDER BY 순으로 평가된다. WHERE 는 그룹이 만들어지기 전에 행을 거르는 단계라 집계 함수를 쓸 수 없고, 집계 결과로 걸러내려면 GROUP BY 뒤의 HAVING 을 써야 한다.",
    "_source": "authored",
    "references": [
      {
        "type": "sql",
        "code": "SELECT DEPTNO, SUM(SAL) AS 합계\nFROM   EMP\nWHERE  SUM(SAL) > 1000\nGROUP BY DEPTNO;"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "correctIndex": 3,
    "explanation": "`mgr = PRIOR empno` 는 이전(부모) 행의 empno 가 현재 행의 mgr 와 같다는 뜻으로 부모→자식 순방향 전개이다. START WITH mgr IS NULL 로 루트(100)에서 시작하므로 PRIOR 는 ㄹ(empno 앞)에 와야 한다. ㄷ(mgr 앞)에 오면 역방향이라 루트 한 행만 조회된다. ㄱ·ㄴ 은 FROM·START WITH 위치라 PRIOR 가 올 수 없다.",
    "_source": "authored",
    "references": [
      {
        "type": "table",
      ...
      {
        "type": "sql",
        "code": "SELECT empno, mgr\nFROM   EMP  -- (ㄱ)\nSTART WITH mgr IS NULL  -- (ㄴ)\nCONNECT BY ( ㄷ ) mgr = ( ㄹ ) empno;"
      }
    ]
  },
  {
