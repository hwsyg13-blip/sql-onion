...
| 결과 | 건수 |
|---|---|
| ❌ 수정 필요 (정답 노출/중복 정답) | **3건** (#26, #43, #47) |
| 🔧 의문 → 수정 (사용자 추가 요청) | **3건** (#4, #25, #39) |
| ✅ 정상 | 44건 |
...
```
서비스 ||--o{ 서비스이용 : "이용"
청구 ||--o{ 서비스이용 : "기준"   ← 청구가 서비스이용의 부모로 표현됨 (어색)
청구 ||--o{ 납부 : "수행"
```

업무 흐름(서비스이용 → 청구 발행 → 납부)에 맞춰 청구·서비스이용 관계 방향 수정:
...
### #25 단일 행 보장 제약 — title·explanation 명확화

기존 title은 "T2.COL2가 가져야 할 제약"이었으나, 단일성 보장은 검색 키 컬럼(WHERE 절의 비교 대상)에 걸려야 함. T2.COL2는 SELECT 절의 반환 컬럼이라 단일성과 무관.

```diff
  title:
-   "아래 SQL이 오류 없이 실행되기 위해 T2.COL2가 가져야 할 제약 조건은?"
+   "아래 SQL의 서브쿼리가 항상 1건 이하만 반환하도록 보장하기 위해 T2.A 컬럼이 가져야 할 제약 조건은?"

  explanation: 검색 키인 T2.A 가 UNIQUE 여야 하는 논리로 명확히 재서술.

  table caption:
-   "T2 테이블 (A='A' 행이 둘 이상이면 단일 행 서브쿼리 오류)"   ← 풀이 단서 노출
+   "T2 테이블"
```

### #39 MERGE 결과 — title에 행 명시

```diff
  title:
-   "아래 TGT, SRC 테이블에 대해 MERGE 문 수행 후 TGT 의 V1, V2, V3 값으로 옳은 것은?"
+   "아래 TGT, SRC 테이블에 대해 MERGE 문 수행 후 TGT의 ID=2 행의 V1, V2, V3 값으로 옳은 것은?"
```

INSERT된 새 행(ID=2)을 묻는 것임을 명시해 모호함 제거.
